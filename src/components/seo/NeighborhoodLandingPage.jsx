...
export default function NeighborhoodLandingPage({ config }) {
  const navigate = useNavigate();
  const { neighborhood, city, state, title, metaTitle, metaDescription, keywords, h1, intro, canonicalUrl } = config;

  const { data: sales = [], isLoading } = useQuery({
    queryKey: ['neighborhoodSales', neighborhood, city],
    queryFn: async () => {
      const allSales = await base44.entities.YardSale.filter({ status: 'approved' }, '-date', 50);
      const now = new Date();
      return allSales.filter((sale) => {
        const loc = (sale.general_location || '').toLowerCase();
        const saleCity = (sale.city || '').toLowerCase();
        const hood = neighborhood.toLowerCase();
        const matchesHood = loc.includes(hood) || saleCity.includes(hood) ||
          loc.includes(city.toLowerCase()) || saleCity.includes(city.toLowerCase());
        const saleEnd = new Date(`${sale.date}T${sale.end_time || '23:59'}`);
        return matchesHood && saleEnd >= now;
      });
    }
  });
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="inline-flex items-center gap-2 bg-white/10 rounded-full px-4 py-2 text-sm font-medium mb-6">
              <MapPin className="w-4 h-4" /> {neighborhood}, {city}
            </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <section className="mb-16">
          ...
          {isLoading ? (
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-12 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-lg mb-4">No upcoming sales in {neighborhood} right now</p>
        ...
        <section className="prose prose-lg max-w-none dark:prose-invert mb-16">
          <h2>About {neighborhood} Stoop & Yard Sales</h2>
          <p>
            {neighborhood} is one of {city}'s most active neighborhoods for stoop sales, garage sales, and estate sales.
            Residents regularly sell furniture, vintage clothing, electronics, antiques, and everyday items at street-level prices.
            Stooplify makes it easy to find all upcoming {neighborhood} sales in one place.
          </p>
          <p>
            Browse the map, set smart alerts for new sales in {neighborhood}, or{' '}
            <Link to={createPageUrl('AddYardSale')} className="text-[#14B8FF]">list your own sale for free</Link> in under 5 minutes.
          </p>
        </section>

        <div className="bg-gradient-to-r from-[#14B8FF] to-[#0da3e6] rounded-2xl p-8 text-white text-center">
          <h3 className="text-2xl font-bold mb-2" style={{ fontFamily: 'Poppins, sans-serif' }}>Have a sale in {neighborhood}?</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Button onClick={() => navigate(createPageUrl('YardSales'))} variant="outline" className="border-white text-white hover:bg-white/10">
              Browse Nearby Sales
            </Button>
          </div>
        </div>
