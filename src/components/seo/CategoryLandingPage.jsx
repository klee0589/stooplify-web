...
export default function CategoryLandingPage({ config }) {
  const navigate = useNavigate();
  const { category, title, metaTitle, metaDescription, keywords, h1, intro, canonicalUrl } = config;

  const { data: sales = [], isLoading } = useQuery({
    queryKey: ['categorySales', category],
    queryFn: async () => {
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="bg-gradient-to-r from-[#14B8FF] to-[#0da3e6] rounded-2xl p-8 text-white text-center">
          <h3 className="text-2xl font-bold mb-2" style={{ fontFamily: 'Poppins, sans-serif' }}>Selling {title}?</h3>
          <p className="text-white/90 mb-6">List for free and reach buyers searching specifically for {title.toLowerCase()}</p>
