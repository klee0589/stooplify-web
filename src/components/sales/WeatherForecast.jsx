  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then(data => {
        ...
        if (match) setSelectedDay(match.date);
        else setSelectedDay(days[0]?.date);
      })
              ...
              {forecast.map(day => {
                ...
                return (
                  <button
                    ...
                    className={`flex-shrink-0 flex flex-col items-center gap-1 px-3 py-2 rounded-xl border-2 transition-all min-w-[56px] ${
                      isSelected
                        ? 'border-[#14B8FF] bg-white shadow-sm'
                        : 'border-transparent bg-white/50 hover:bg-white/80'
                    } ${isSaleDate ? 'ring-2 ring-[#FF6F61] ring-offset-1' : ''}`}
                  >
